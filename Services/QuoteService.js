...
var getOb = function (criteria, projection, options, callback) {
    options.lean = true; 
    Models.Quote.find(criteria, projection, options, callback);
};

//Insert Account Data in DB
var createOb = function (objToSave, callback) {
    new Models.Quote(objToSave).save(function(err, resp){
            if(err)
                return callback(err);
            callback(err, resp)
    })
...
var updateOb = function (criteria, dataToSet, options, callback) {
    options.lean = true;
    options.new = true;
    Models.Quote.findOneAndUpdate(criteria, dataToSet, options, callback);
};

//Delete Account from DB
var deleteOb = function (criteria, callback) {
    Models.Quote.delete(criteria,function(err,resp){
        if(err)
            return callback(err)
        if(resp)
